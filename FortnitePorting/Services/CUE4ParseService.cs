...
public partial class CUE4ParseService : ObservableObject, IService, IResettable
{
    ...
    {
        // Recursive. GetFiles() with no arguments only looks at the top level, so nothing under
        // .cache/uondemandtoc/<BuildVersion>/ was ever expired — a stale or broken toc stayed
        // cached indefinitely, and every Fortnite build a user has ever loaded kept its own
        // folder of ~60MB tocs forever.
        var files = CacheFolder.GetFiles("*", SearchOption.AllDirectories);
        ...
        foreach (var file in files)
        {
            ...
            try { file.Delete(); } catch { /* in use or gone; it'll be caught next launch */ }
        }

        // Then drop the build-version folders the prune just emptied, deepest first.
        foreach (var directory in CacheFolder.GetDirectories("*", SearchOption.AllDirectories)
