...
public partial class CUE4ParseService : ObservableObject, IService
{
    ...
    private async Task InitializeProviderSetup()
    {
        ...
        {
            // Detex.dll is Windows-only. Route BC1-BC7 textures through the managed AssetRipper decoder
            // so cosmetic icons (typically BC7) render without it. ETC formats still need Detex but
            // aren't used by Fortnite cosmetics on PC.
    ...
    private async Task InitializeDetex()
    {
        if (!OperatingSystem.IsWindows())
        {
            // Detex is shipped as a Windows-only DLL. Remove any stale copy a prior run may have written
            // and skip initialization — only ETC/BPTC textures depend on it.
